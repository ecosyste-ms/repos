namespace :sidekiq do
  namespace :unique_jobs do
    ...
    desc "Clear unique job locks matching a pattern (e.g. PATTERN='*SyncRepository*')"
    task clear_matching: :environment do
      pattern = ENV.fetch("PATTERN") { abort "Usage: rake sidekiq:unique_jobs:clear_matching PATTERN='*SyncRepository*'" }
      digests = SidekiqUniqueJobs::Digests.new
